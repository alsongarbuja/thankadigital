...
export default function WebDevCourseRegisterPage() {
  ...
  const [currentField, setCurrentField] = useState(
    "entry.668855873" as keyof userData
  );
  ...
  const handleContinue = () => {
    if (
      data["entry.668855873"] === "" ||
      data["entry.2093799469"] === "" ||
      data["entry.148672612"] === "" ||
      data["entry.331549010"] === "" ||
      data["entry.64164442"] === ""
    ) {
      setError((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    if (currentField === "entry.703362708") {
      localStorage.setItem(
        "thankadigital-web-course-registration",
        JSON.stringify(data)
      );
    }
  }, [currentField, data]);
  ...
  return (
    <>
        ...
        <div className="fixed top-0 left-0 z-20 flex items-center justify-center w-full h-screen bg-black/50">
          <div className="w-[90%] lg:w-1/2 h-[80%] overflow-y-scroll px-6 py-10 bg-white rounded-md">
            ...
            <p className="italic font-bold text-red-500">
              Note: Read all of this before continue
            </p>
            <p className="my-2">
              Discover the comprehensive web development crash course offered by
              Thanka Digital at the convenient Fulbari 11 location. Designed to
              empower your skills, this 40-day program focuses on the dynamic
              MERN stack technology. MERN stack is one of the tech stack used in
              the web development community with its dynamic nature and easy to
              configure and use.
              <br />
            ...
            <ol className="pl-8 list-decimal">
              ...
              <li>
                React indepth (hooks, state management, HOC, Routing, custom
                hooks, forms, etc)
              </li>
              <li>Node Basic (CommonJS syntax, fs, backend development)</li>
              <li>
                Express (server using express, middlewears, error handling, API
                creation, etc)
              </li>
              <li>
                MongoDB (NOSQL concepts, Document and collections, moongose,
            ...
            <p className="my-4">
              Join us on this journey of learning and growth for a nominal fee
              of NPRS 14000, and unlock the potential to create impactful web
              solutions.
            </p>

            <button
              onClick={() => setIsFirstOpen(false)}
              className="p-2 text-green-500 bg-green-200 rounded-md"
            >
              Continue
            </button>
          </div>
      ...
      {!isFirstOpen && hasResponse && (
        <div className="fixed top-0 left-0 z-20 flex items-center justify-center w-full h-screen bg-black/50">
          <div className="w-[90%] lg:w-1/2 px-6 py-10 bg-white rounded-md">
            ...
            <p className="my-2">
              If you want to update your register, please contact us at{" "}
              <a
                href="mailto:thankadigital@gmail.com"
                className="font-bold underline text-primary_blue"
              >
                thankadigital@gmail.com
              </a>
            </p>
          </div>
        </div>
      ...
      <main className="flex flex-col items-center justify-center w-full min-h-screen gap-8 p-10">
        <h1>
          <span className="text-primary_red">Thanka</span>{" "}
          <span className="text-primary_blue">Digital</span>
        </h1>
        ...
        <form
          action="https://docs.google.com/forms/d/e/1FAIpQLSem27B9cJzDg_kDEX81Z-UXMx6fgN1Aok8wcA2VZ_bgd3I17w/formResponse"
          className="w-[80%]"
        >
          <Input
            label="Full Name"
  ...
  return (
    <div className="flex justify-center">
      {steps.map((step) => (
        <div
          key={step.stepIndex}
          className={`flex flex-col justify-center mb-4 border-t ${
            currentStep >= step.stepIndex ? "border-green-600" : "border-black"
          } min-h-[100px] relative`}
        >
          <div
            className={`absolute -top-5 flex items-center justify-center w-10 h-10 text-white ${
              currentStep >= step.stepIndex ? "bg-green-600" : "bg-black"
